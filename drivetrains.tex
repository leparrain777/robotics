...
\begin{itemize}
    ...
    \item 
    \item Needs space for extra gearboxes and power transmission.
    \item Varying difficullty to build. Ranked from easy to medium: drift, kiwi, holonomic, then H-drive.
\end{itemize}
\subsection{Assembly and Repair}
\begin{itemize}
    \item Assembyly may or may not be tight on space depending on the design.
    \item Somewhat harder to repair.
    \item One spare required at all times, possibly two depending on level of pushing going on.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Crab and Swerve Drive}
\subsection{Cost}
\begin{itemize}
    \item Needs 5 or 8 gearboxes respectively usually.
    \item Motors used is 5 or 8 respectively, where 0 or 4 can be intermitent respectively usually.
    \item Wheels used is 4 usually.
    \item Wheels need to be relatively small due to weight and space concerns.
    \item Each wheel needs lots of extra hardware to function properly.
    \item Must use gears for wheel power,but can use chain or pullies for crab steering, where swerve requires gears for steering.
\end{itemize}
\subsection{Programming}
\begin{itemize}
    \item Can be done by seniors with a lot of help for crab, most likely swerve would be written by mentors or professional libraries.
    \item Programming requires knowlege of trigonometry, and matrix math for swerve.
    \item Extremely long development time, often longer than a full season for swerve.
    \item Surprisingly easy to program for autonomous. Dead reckoning is almost guaranteed.
    \item Needs tons of encoders and sensors, and is often used with a gyro.
    \item Very large risk of programming bugs, and possible physical damage from bad code.
\end{itemize}
\subsection{Capabilities}
\begin{itemize}
    \item The most efficient way to push in any direction.
    \item Resists all pushing in any direction with driver skill with crab, resists all pushing with no driver skill in swerve.
    \item Rotation hard to program but very doable with swerve.
    \item Complete traction reliability. 
    \item Ideal for very movement heavy games where pushing, speed, and evasion are central.
\end{itemize}
\subsection{Hinderances}
\begin{itemize}
    \item For crab, rotation not possible except by running into things which could cause problems if orientation is important.
    \item Gyro can cause major problems if it is wrong.
\end{itemize}
\subsection{Driver Usage}
\begin{itemize}
    \item Easy to learn and be great at but hard to master with crab, easy to learn and master with swerve.
    \item Possible orientation corrections based on presence of gyro/accelerometer and its drift.
    \item Most likely there will not be a practice bot ready.
    \item Harder control scheme, often uses at least 3 axis on the same joystick where human error is likely, but is intuitive.
    \item Unlikely to have experienced drivers, but it isn't a problem as it is intuitive.
\end{itemize}
\subsection{Design and Build}
\begin{itemize}
    \item Incredibly hard to design, but expensive premade modules are available for purchase.
    \item Needs a lot of extra space and weight allowance for extra gearboxes and power transmission.
    \item Hard to build, especially swerve.
\end{itemize}
\subsection{Assembly and Repair}
\begin{itemize}
    \item A nightmare to assemble and requires special tools most likely.
    \item Near impossible to repair in a reasonable amount of time.
    \item Spares required at all times for basically the entire drive base.
\end{itemize}
